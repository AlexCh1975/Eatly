<template>
  <div class="home">
    <div class="home__top center">
      <div class="top-left">
        <div class="top-left__box">
          <p class="top-left__line"></p>
          <p class="top-left__over">over 1000 users</p>
        </div>
        <h1 class="top-left__title">
          Enjoy Foods All<br>
          Over The <span>World</span>
        </h1>
        <p class="top-left__text">
          EatLy help you set saving goals, earn cash back offers, Go to disclaimer for more details and get paychecks up
          to two days early. Get a <span>$20 bonus.</span>
        </p>
        <div class="top-left__btns">
          <button class="top-left__btns_started">Get Started</button>
          <button class="top-left__btns_goPro">Go Pro</button>
        </div>
        <div class="top-left__stars">
          <svg class="top-left__stars_svg" xmlns="http://www.w3.org/2000/svg" width="123" height="31" viewBox="0 0 123 31"
            fill="none">
            <path
              d="M32.3291 10.7012H44.7726V13.0177H39.8731V26.0762H37.1876V13.0177H32.3086L32.3291 10.7012ZM44.2396 14.9447H46.5356V17.0972H46.5766C46.6586 16.7897 46.8021 16.5027 47.0071 16.2157C47.2121 15.9287 47.4581 15.6622 47.7656 15.4367C48.0526 15.1907 48.3806 15.0062 48.7496 14.8627C49.0981 14.7192 49.4671 14.6372 49.8361 14.6372C50.1231 14.6372 50.3281 14.6577 50.4306 14.6577C50.5331 14.6782 50.6561 14.6987 50.7586 14.6987V17.0562C50.5946 17.0152 50.4101 16.9947 50.2256 16.9742C50.0411 16.9537 49.8771 16.9332 49.6926 16.9332C49.2826 16.9332 48.8931 17.0152 48.5241 17.1792C48.1551 17.3432 47.8476 17.5892 47.5606 17.8967C47.2941 18.2247 47.0686 18.6142 46.9046 19.0857C46.7406 19.5572 46.6586 20.1107 46.6586 20.7257V26.0147H44.1986L44.2396 14.9447ZM62.0336 26.0762H59.6146V24.5182H59.5736C59.2661 25.0922 58.8151 25.5227 58.2206 25.8712C57.6261 26.2197 57.0111 26.3837 56.3961 26.3837C54.9406 26.3837 53.8746 26.0352 53.2186 25.2972C52.5626 24.5797 52.2346 23.4727 52.2346 22.0172V14.9447H54.6946V21.7712C54.6946 22.7552 54.8791 23.4522 55.2686 23.8417C55.6376 24.2517 56.1706 24.4567 56.8471 24.4567C57.3596 24.4567 57.7901 24.3747 58.1386 24.2107C58.4871 24.0467 58.7741 23.8417 58.9791 23.5752C59.2046 23.3087 59.3481 22.9807 59.4506 22.6117C59.5531 22.2427 59.5941 21.8327 59.5941 21.4022V14.9447H62.0541V26.0762H62.0336ZM66.2156 22.5092C66.2976 23.2267 66.5641 23.7187 67.0356 24.0262C67.5071 24.3132 68.0811 24.4567 68.7371 24.4567C68.9626 24.4567 69.2291 24.4362 69.5161 24.3952C69.8031 24.3542 70.0901 24.2927 70.3361 24.1902C70.6026 24.0877 70.8076 23.9442 70.9716 23.7392C71.1356 23.5342 71.2176 23.2882 71.1971 22.9807C71.1766 22.6732 71.0741 22.4067 70.8486 22.2017C70.6231 21.9967 70.3566 21.8532 70.0286 21.7097C69.7006 21.5867 69.3111 21.4842 68.8806 21.4022C68.4501 21.3202 68.0196 21.2177 67.5686 21.1152C67.1176 21.0127 66.6666 20.8897 66.2361 20.7462C65.8056 20.6027 65.4366 20.4182 65.0881 20.1517C64.7601 19.9057 64.4731 19.5982 64.2886 19.2087C64.0836 18.8192 63.9811 18.3682 63.9811 17.7942C63.9811 17.1792 64.1246 16.6872 64.4321 16.2772C64.7191 15.8672 65.1086 15.5392 65.5596 15.2932C66.0106 15.0472 66.5231 14.8627 67.0766 14.7602C67.6301 14.6577 68.1631 14.6167 68.6551 14.6167C69.2291 14.6167 69.7826 14.6782 70.2951 14.8012C70.8076 14.9242 71.2996 15.1087 71.7096 15.3957C72.1401 15.6622 72.4886 16.0312 72.7551 16.4617C73.0421 16.8922 73.2061 17.4252 73.2881 18.0402H70.7256C70.6026 17.4457 70.3361 17.0562 69.9261 16.8512C69.4956 16.6462 69.0241 16.5437 68.4706 16.5437C68.3066 16.5437 68.0811 16.5642 67.8556 16.5847C67.6096 16.6257 67.4046 16.6667 67.1791 16.7487C66.9741 16.8307 66.7896 16.9537 66.6461 17.0972C66.5026 17.2407 66.4206 17.4457 66.4206 17.6917C66.4206 17.9992 66.5231 18.2247 66.7281 18.4092C66.9331 18.5937 67.1996 18.7372 67.5481 18.8807C67.8761 19.0037 68.2656 19.1062 68.6961 19.1882C69.1266 19.2702 69.5776 19.3727 70.0286 19.4752C70.4796 19.5777 70.9101 19.7007 71.3406 19.8442C71.7711 19.9877 72.1606 20.1722 72.4886 20.4387C72.8166 20.6847 73.1036 20.9922 73.3086 21.3612C73.5136 21.7302 73.6161 22.2017 73.6161 22.7347C73.6161 23.3907 73.4726 23.9442 73.1651 24.4157C72.8576 24.8667 72.4681 25.2562 71.9966 25.5432C71.5251 25.8302 70.9716 26.0352 70.3976 26.1787C69.8031 26.3017 69.2291 26.3837 68.6551 26.3837C67.9581 26.3837 67.3021 26.3017 66.7076 26.1377C66.1131 25.9737 65.5801 25.7277 65.1496 25.4202C64.7191 25.0922 64.3706 24.7027 64.1246 24.2107C63.8786 23.7392 63.7351 23.1652 63.7146 22.4887H66.1951V22.5092H66.2156ZM74.3131 14.9447H76.1786V11.5827H78.6386V14.9242H80.8526V16.7487H78.6386V22.6937C78.6386 22.9602 78.6591 23.1652 78.6796 23.3702C78.7001 23.5547 78.7616 23.7187 78.8231 23.8417C78.9051 23.9647 79.0281 24.0672 79.1716 24.1287C79.3356 24.1902 79.5406 24.2312 79.8276 24.2312C79.9916 24.2312 80.1761 24.2312 80.3401 24.2107C80.5041 24.1902 80.6886 24.1697 80.8526 24.1287V26.0352C80.5861 26.0762 80.3196 26.0967 80.0531 26.1172C79.7866 26.1582 79.5406 26.1582 79.2536 26.1582C78.5976 26.1582 78.0851 26.0967 77.6956 25.9737C77.3061 25.8507 76.9986 25.6662 76.7731 25.4407C76.5476 25.1947 76.4041 24.9077 76.3221 24.5592C76.2401 24.2107 76.1991 23.7802 76.1786 23.3292V16.7692H74.3131V14.9037V14.9447ZM82.5746 14.9447H84.8911V16.4617H84.9321C85.2806 15.8057 85.7521 15.3547 86.3671 15.0677C86.9821 14.7807 87.6381 14.6372 88.3761 14.6372C89.2576 14.6372 90.0161 14.7807 90.6721 15.1087C91.3281 15.4162 91.8611 15.8262 92.2916 16.3797C92.7221 16.9127 93.0501 17.5482 93.2551 18.2657C93.4601 18.9832 93.5831 19.7622 93.5831 20.5617C93.5831 21.3202 93.4806 22.0377 93.2961 22.7347C93.0911 23.4317 92.8041 24.0672 92.4146 24.6002C92.0251 25.1332 91.5331 25.5637 90.9181 25.8917C90.3031 26.2197 89.6061 26.3837 88.7861 26.3837C88.4376 26.3837 88.0686 26.3427 87.7201 26.2812C87.3716 26.2197 87.0231 26.1172 86.6951 25.9737C86.3671 25.8302 86.0596 25.6457 85.7931 25.4202C85.5061 25.1947 85.2806 24.9282 85.0961 24.6412H85.0551V30.1967H82.5951V14.9447H82.5746ZM91.1641 20.5207C91.1641 20.0287 91.1026 19.5367 90.9591 19.0652C90.8361 18.5937 90.6311 18.1837 90.3646 17.8147C90.0981 17.4457 89.7701 17.1587 89.4011 16.9332C89.0116 16.7077 88.5606 16.6052 88.0686 16.6052C87.0436 16.6052 86.2646 16.9537 85.7316 17.6712C85.2191 18.3887 84.9526 19.3522 84.9526 20.5412C84.9526 21.1152 85.0141 21.6277 85.1576 22.0992C85.3011 22.5707 85.4856 22.9807 85.7726 23.3292C86.0391 23.6777 86.3671 23.9442 86.7566 24.1492C87.1461 24.3542 87.5766 24.4567 88.0891 24.4567C88.6631 24.4567 89.1141 24.3337 89.5036 24.1082C89.8931 23.8827 90.2006 23.5752 90.4671 23.2267C90.7131 22.8577 90.8976 22.4477 91.0001 21.9762C91.1026 21.4842 91.1641 21.0127 91.1641 20.5207ZM95.4896 10.7012H97.9496V13.0177H95.4896V10.7012ZM95.4896 14.9447H97.9496V26.0762H95.4896V14.9447ZM100.143 10.7012H102.603V26.0762H100.143V10.7012ZM110.106 26.3837C109.225 26.3837 108.425 26.2402 107.728 25.9327C107.031 25.6252 106.457 25.2357 105.965 24.7232C105.494 24.2107 105.125 23.5752 104.879 22.8577C104.633 22.1402 104.489 21.3407 104.489 20.5002C104.489 19.6597 104.612 18.8807 104.879 18.1632C105.125 17.4457 105.494 16.8307 105.965 16.2977C106.437 15.7852 107.031 15.3752 107.728 15.0882C108.425 14.8012 109.225 14.6372 110.106 14.6372C110.988 14.6372 111.787 14.7807 112.484 15.0882C113.181 15.3752 113.755 15.7852 114.247 16.2977C114.719 16.8102 115.088 17.4457 115.334 18.1632C115.58 18.8807 115.723 19.6597 115.723 20.5002C115.723 21.3612 115.6 22.1402 115.334 22.8577C115.067 23.5752 114.719 24.1902 114.247 24.7232C113.776 25.2357 113.181 25.6457 112.484 25.9327C111.787 26.2197 111.008 26.3837 110.106 26.3837ZM110.106 24.4362C110.639 24.4362 111.131 24.3132 111.521 24.0877C111.931 23.8622 112.238 23.5547 112.505 23.1857C112.771 22.8167 112.956 22.3862 113.079 21.9352C113.202 21.4637 113.263 20.9922 113.263 20.5002C113.263 20.0287 113.202 19.5572 113.079 19.0857C112.956 18.6142 112.771 18.2042 112.505 17.8352C112.238 17.4662 111.91 17.1792 111.521 16.9537C111.111 16.7282 110.639 16.6052 110.106 16.6052C109.573 16.6052 109.081 16.7282 108.692 16.9537C108.282 17.1792 107.974 17.4867 107.708 17.8352C107.441 18.2042 107.257 18.6142 107.134 19.0857C107.011 19.5572 106.949 20.0287 106.949 20.5002C106.949 20.9922 107.011 21.4637 107.134 21.9352C107.257 22.4067 107.441 22.8167 107.708 23.1857C107.974 23.5547 108.302 23.8622 108.692 24.0877C109.102 24.3337 109.573 24.4362 110.106 24.4362ZM116.461 14.9447H118.327V11.5827H120.787V14.9242H123.001V16.7487H120.787V22.6937C120.787 22.9602 120.807 23.1652 120.828 23.3702C120.848 23.5547 120.91 23.7187 120.971 23.8417C121.053 23.9647 121.176 24.0672 121.32 24.1287C121.484 24.1902 121.689 24.2312 121.976 24.2312C122.14 24.2312 122.324 24.2312 122.488 24.2107C122.652 24.1902 122.837 24.1697 123.001 24.1287V26.0352C122.734 26.0762 122.468 26.0967 122.201 26.1172C121.935 26.1582 121.689 26.1582 121.402 26.1582C120.746 26.1582 120.233 26.0967 119.844 25.9737C119.454 25.8507 119.147 25.6662 118.921 25.4407C118.696 25.1947 118.552 24.9077 118.47 24.5592C118.388 24.2107 118.347 23.7802 118.327 23.3292V16.7692H116.461V14.9037V14.9447Z"
              fill="black" />
            <path
              d="M29.4585 10.701H18.204L14.7395 0L11.2545 10.701L0 10.6805L9.10199 17.302L5.617 28.003L14.719 21.3815L23.821 28.003L20.3565 17.302L29.4585 10.701Z"
              fill="#00B67A" />
            <path d="M21.1362 19.7208L20.3572 17.3018L14.7402 21.3813L21.1362 19.7208Z" fill="#005128" />
          </svg>
          <svg class="top-left__stars_svg-balls" xmlns="http://www.w3.org/2000/svg" width="96" height="16"
            viewBox="0 0 96 16" fill="none">
            <g opacity="0.8">
              <path
                d="M6.95816 0.741543C7.27942 -0.247181 8.6782 -0.247181 8.99945 0.741543L10.1473 4.2742C10.291 4.71637 10.703 5.01575 11.1679 5.01575H14.8824C15.922 5.01575 16.3542 6.34607 15.5132 6.95713L12.5081 9.14044C12.132 9.41371 11.9746 9.89811 12.1183 10.3403L13.2661 13.8729C13.5874 14.8617 12.4557 15.6838 11.6147 15.0728L8.6096 12.8895C8.23347 12.6162 7.72415 12.6162 7.34801 12.8895L4.34295 15.0728C3.5019 15.6838 2.37026 14.8617 2.69151 13.8729L3.83935 10.3403C3.98302 9.89811 3.82563 9.41371 3.44949 9.14044L0.444434 6.95713C-0.396625 6.34607 0.0356224 5.01575 1.07523 5.01575H4.78969C5.25461 5.01575 5.66666 4.71637 5.81033 4.2742L6.95816 0.741543Z"
                fill="#F59E0B" />
              <path
                d="M26.9152 0.741543C27.2364 -0.247181 28.6352 -0.247181 28.9565 0.741543L30.1043 4.2742C30.248 4.71637 30.66 5.01575 31.125 5.01575H34.8394C35.879 5.01575 36.3113 6.34607 35.4702 6.95713L32.4652 9.14044C32.089 9.41371 31.9316 9.89811 32.0753 10.3403L33.2231 13.8729C33.5444 14.8617 32.4128 15.6838 31.5717 15.0728L28.5666 12.8895C28.1905 12.6162 27.6812 12.6162 27.305 12.8895L24.3 15.0728C23.4589 15.6838 22.3273 14.8617 22.6485 13.8729L23.7964 10.3403C23.94 9.89811 23.7827 9.41371 23.4065 9.14044L20.4015 6.95713C19.5604 6.34607 19.9927 5.01575 21.0323 5.01575H24.7467C25.2116 5.01575 25.6237 4.71637 25.7674 4.2742L26.9152 0.741543Z"
                fill="#F59E0B" />
              <path
                d="M46.8722 0.741543C47.1935 -0.247181 48.5923 -0.247181 48.9135 0.741543L50.0613 4.2742C50.205 4.71637 50.6171 5.01575 51.082 5.01575H54.7965C55.8361 5.01575 56.2683 6.34607 55.4272 6.95713L52.4222 9.14044C52.0461 9.41371 51.8887 9.89811 52.0323 10.3403L53.1802 13.8729C53.5014 14.8617 52.3698 15.6838 51.5287 15.0728L48.5237 12.8895C48.1475 12.6162 47.6382 12.6162 47.2621 12.8895L44.257 15.0728C43.416 15.6838 42.2843 14.8617 42.6056 13.8729L43.7534 10.3403C43.8971 9.89811 43.7397 9.41371 43.3636 9.14044L40.3585 6.95713C39.5174 6.34607 39.9497 5.01575 40.9893 5.01575H44.7037C45.1687 5.01575 45.5807 4.71637 45.7244 4.2742L46.8722 0.741543Z"
                fill="#F59E0B" />
              <path
                d="M66.8312 0.741543C67.1525 -0.247181 68.5512 -0.247181 68.8725 0.741543L70.0203 4.2742C70.164 4.71637 70.5761 5.01575 71.041 5.01575H74.7554C75.795 5.01575 76.2273 6.34607 75.3862 6.95713L72.3812 9.14044C72.005 9.41371 71.8476 9.89811 71.9913 10.3403L73.1391 13.8729C73.4604 14.8617 72.3288 15.6838 71.4877 15.0728L68.4826 12.8895C68.1065 12.6162 67.5972 12.6162 67.2211 12.8895L64.216 15.0728C63.3749 15.6838 62.2433 14.8617 62.5646 13.8729L63.7124 10.3403C63.8561 9.89811 63.6987 9.41371 63.3225 9.14044L60.3175 6.95713C59.4764 6.34607 59.9087 5.01575 60.9483 5.01575H64.6627C65.1277 5.01575 65.5397 4.71637 65.6834 4.2742L66.8312 0.741543Z"
                fill="#F59E0B" />
              <path
                d="M86.7882 0.741543C87.1095 -0.247181 88.5083 -0.247181 88.8295 0.741543L89.9774 4.2742C90.121 4.71637 90.5331 5.01575 90.998 5.01575H94.7125C95.7521 5.01575 96.1843 6.34607 95.3433 6.95713L92.3382 9.14044C91.9621 9.41371 91.8047 9.89811 91.9483 10.3403L93.0962 13.8729C93.4174 14.8617 92.2858 15.6838 91.4447 15.0728L88.4397 12.8895C88.0635 12.6162 87.5542 12.6162 87.1781 12.8895L84.173 15.0728C83.332 15.6838 82.2003 14.8617 82.5216 13.8729L83.6694 10.3403C83.8131 9.89811 83.6557 9.41371 83.2796 9.14044L80.2745 6.95713C79.4335 6.34607 79.8657 5.01575 80.9053 5.01575H84.6198C85.0847 5.01575 85.4967 4.71637 85.6404 4.2742L86.7882 0.741543Z"
                fill="#F59E0B" />
            </g>
          </svg>
          <p class="top-left__stars_balls">4900<span>+</span></p>
        </div>
      </div>
      <div class="top-right">
        <div class="top-right__img">
          <img :src="require(`@/assets/images/FoodTop.png`)" 
                alt="retaurans pic" 
          />
        </div>
        <div class="top-right__card">
          <OrdersCard :card="this.GET_ORDERS[0]"/>
        </div>
        <div class="top-right__graph">
          <div class="top-right__graph_main">
            <p class="top-right__graph_el">Main</p>
            <p class="top-right__graph_el">Weekly</p>
            <p class="top-right__graph_el">Monthly</p>
            <p class="top-right__graph_el">Yearly</p>
          </div>
          <div class="top-right__graph_line">
            <svg class="top-right__graph_svg" xmlns="http://www.w3.org/2000/svg" width="209" height="100" viewBox="0 0 209 100" fill="none">
            <path d="M1.07422 83.6715C12.1442 83.6715 13.2155 98.0846 24.9997 98.0846C36.7839 98.0846 33.9271 57.1211 44.9972 57.1211C56.0672 57.1211 49.9965 22.6056 65.3517 22.6056C80.7069 22.6056 73.5649 57.1211 87.4917 57.1211C101.418 57.1211 99.2759 16.8297 108.203 16.8297C117.131 16.8297 114.631 40.8116 125.344 40.8116C136.057 40.8116 131.058 1.25586 142.485 1.25586C153.912 1.25586 152.84 18.5225 163.196 18.5225C173.552 18.5225 174.623 38.5359 188.907 38.5359C203.191 38.5359 194.264 60.8428 207.833 60.8428" stroke="#6C5FBC" stroke-width="1.98779" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <div class="top-right__graph_day">
            <p class="top-right__graph_item">2D</p>
            <p class="top-right__graph_item">4D</p>
            <p class="top-right__graph_item">5D</p>
            <p class="top-right__graph_item">6D</p>
            <p class="top-right__graph_item">7D</p>
          </div>
        </div>
      </div>
    </div>
    <div class="features">
      <div class="features__item">
        <h2 class="features__heading">10K+</h2>
        <p class="features__text">
          Satisfied Costumers<br>
          All Great Over The World 
        </p>
      </div>
      <div class="features__item">
        <h2 class="features__heading">4M</h2>
        <p class="features__text">
          Healthy Dishes Sold<br>
          Including Milk Shakes Smooth
        </p>
      </div>
      <div class="features__item">
        <h2 class="features__heading">99.99%</h2>
        <p class="features__text">
          Reliable Customer Support<br>
          We Provide Great Experiences
        </p>
      </div>
    </div>
    <div class="home__top_pseudo-box"></div>
    <div class="dowload center">
      <div class="dowload__img-box">
        <img class="dowload__img" 
          :src="require(`@/assets/images/MobileScreen.png`)"
          :alt="mobile"
        />
      </div>
      <div class="dowload__content">
        <div class="dowload__title">
          Premium <span>Quality</span><br>
          For Your Health
        </div> 
        <ul class="dowload__ul">
          <li class="dowload__li">
            Premium quality food is made with ingredients that are packed with essential vitamins, minerals.
          </li>
          <li class="dowload__li">
            These foods promote overall wellness by support healthy digestion and boosting immunity
          </li>
        </ul>
        <button class="dowload__btn">Download</button>
      </div>
    </div>
    <div class="restaurans center">
      <h2 class="restaurans__title">Our Top <span>Restaurants</span></h2>
      <div class="cardList">
        <RestauransCard v-for="card in this.GET_RESTAURANS" 
          :key="card.id" 
          :card="card" />
      </div>
      <div class="viewAll-box">
        <BtnViewAll />
      </div>
    </div>
    <LineComp class="center"/>
    <div class="dishes center">
      <h2 class="dishes__title">Our Top <span>Dishes</span></h2>
      <div class="dishes__cardList">
        <DishesCard v-for="card in this.GET_DISHES" 
          :key="card.id" 
          :card="card" 
        />
      </div>
      <div class="viewAll-box">
        <BtnViewAll />
      </div>
    </div>
    <LineComp class="center" />
    <div class="dashboard center">
      <div class="dashboard__card">
        <h2 class="dashboard__title">
          Control <span>Purchases</span><br>
          Via Dashboard
        </h2>
        <div class="dashboard__ordersCardList">
          <OrdersCard v-for="card in this.GET_ORDERS"
            :key="card.id" 
            :card="card" 
          />
        </div>
      </div>
      <div class="dashboard__purchases">
        <div class="purchases__top">
          <h3 class="purchases__title">
            Purchases
          </h3>
          <details class="purchases__details">
            <summary class="purchases__summary">
              <span class="purchases__select-title">This month</span>
              <svg class="purchases__select-svg" xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                <path d="M6.4209 10.2529L12.7033 16.5353L18.9856 10.2529" stroke="#1A202C" stroke-width="2.35588" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </summary>
          </details>
        </div>
        <div class="purchases__block expense">
          <div class="purchases__content">
            <div class="purchases__content-box">
              <div class="purchases__icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                <path d="M13.3524 2.77494L13.321 2.84823L10.2845 9.89494H7.30039C6.58839 9.89494 5.9078 10.0415 5.29004 10.3033L7.12239 5.92659L7.16427 5.82188L7.23757 5.65435C7.25851 5.59153 7.27945 5.52871 7.31086 5.47635C8.68251 2.30376 10.2322 1.58129 13.3524 2.77494Z" stroke="#936DFF" stroke-width="1.57059" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M18.9957 10.1043C18.5245 9.95773 18.0219 9.8949 17.5194 9.8949H10.2842L13.3206 2.8482L13.3521 2.7749C13.5091 2.82726 13.6557 2.90055 13.8128 2.96337L16.1268 3.93714C17.4147 4.47114 18.3151 5.02608 18.8596 5.6962C18.9643 5.82184 19.0481 5.93702 19.1214 6.07314C19.2156 6.21973 19.2889 6.36631 19.3308 6.52337C19.3727 6.61761 19.4041 6.71184 19.425 6.79561C19.7077 7.67514 19.5402 8.75361 18.9957 10.1043Z" stroke="#936DFF" stroke-width="1.57059" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M22.6303 15.0052V17.0469C22.6303 17.2563 22.6198 17.4657 22.6093 17.6752C22.4104 21.3294 20.3686 23.1722 16.4945 23.1722H8.32744C8.07615 23.1722 7.82485 23.1513 7.58403 23.1199C4.25438 22.9 2.47438 21.12 2.2545 17.7903C2.22309 17.5495 2.20215 17.2982 2.20215 17.0469V15.0052C2.20215 12.9006 3.47956 11.0892 5.30144 10.3039C5.92968 10.0421 6.5998 9.89551 7.3118 9.89551H17.5311C18.0441 9.89551 18.5467 9.9688 19.0074 10.1049C21.0911 10.7436 22.6303 12.6912 22.6303 15.0052Z" stroke="#936DFF" stroke-width="1.57059" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M7.12208 5.92676L5.28972 10.3035C3.46784 11.0888 2.19043 12.9002 2.19043 15.0048V11.9369C2.19043 8.96323 4.30549 6.4817 7.12208 5.92676Z" stroke="#936DFF" stroke-width="1.57059" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M22.6277 11.9358V15.0036C22.6277 12.7001 21.099 10.7421 19.0049 10.1139C19.5494 8.7527 19.7064 7.6847 19.4446 6.7947C19.4237 6.70046 19.3923 6.60623 19.3504 6.52246C21.2979 7.52764 22.6277 9.59034 22.6277 11.9358Z" stroke="#936DFF" stroke-width="1.57059" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </div>
              <div class="purchases__box">
                <h4 class="purchases__title">Expense</h4>
                <p class="purchases__text">
                  Increased By <span class="purchases__persent expensePersent">10</span>%
                </p>
              </div>
            </div>
            <p class="purchases__price expensePrice">$409.00</p>
          </div>
          <div class="purchases__line expense">
            <div class="purchases__expenseLine"></div>
          </div>
        </div>
        <div class="purchases__block vocher">
          <div class="purchases__content">
            <div class="purchases__content-box">
              <div class="purchases__icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
                <path d="M17.8963 22.2331H7.42572C4.28455 22.2331 2.19043 20.6625 2.19043 16.9978V9.6684C2.19043 6.00369 4.28455 4.43311 7.42572 4.43311H17.8963C21.0375 4.43311 23.1316 6.00369 23.1316 9.6684V16.9978C23.1316 20.6625 21.0375 22.2331 17.8963 22.2331Z" stroke="#22C55E" stroke-width="1.57059" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M12.6617 16.4738C14.3965 16.4738 15.8029 15.0674 15.8029 13.3326C15.8029 11.5978 14.3965 10.1914 12.6617 10.1914C10.9269 10.1914 9.52051 11.5978 9.52051 13.3326C9.52051 15.0674 10.9269 16.4738 12.6617 16.4738Z" stroke="#22C55E" stroke-width="1.57059" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M5.85645 10.7153V15.9506" stroke="#22C55E" stroke-width="1.57059" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M19.4668 10.7153V15.9506" stroke="#22C55E" stroke-width="1.57059" stroke-miterlimit="10" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </div>
              <div class="purchases__box">
                <h4 class="purchases__title">Vocher Usage</h4>
                <p class="purchases__text">
                  Increased By <span class="purchases__persent vocherPersent">5</span>%
                </p>
              </div>
            </div>
            <p class="purchases__price vocherPrice">$45.78</p>
          </div>
          <div class="purchases__line vocher">
            <div class="purchases__vocherLine"></div>
          </div>
        </div>
      </div>
    </div>
    <LineComp class="center" />
    <section class="reviews">
      <h2 class="reviews__title"><span>Customer</span> Say</h2>
      <div class="reviews__wrapper">
        <div class="reviews__container">
          <div class="reviews__content center">
            <div class="reviews__body isActive">
              <ReviewComp :review="this.GET_REVIEWS[0]"/>
            </div>  
          </div>
        </div>
        <div class="reviews__box">
          <div class="reviews__box_el">
            <ReviewComp v-for="review in this.GET_REVIEWS" 
              :key="review.id"
              :review="review"
            />
          </div>
            <!-- <ReviewComp v-for="review in this.GET_REVIEWS" 
              :key="review.id"
              :review="review"
            /> -->
          
            <input class="reviews__range" type="range" id="reviews__range" name="volume" min="0" max="11" />
            <!-- <label for="reviews__range">Volume</label> -->
        
        </div> 
      </div>
      <!-- <div class="reviews__container">
        <div class="reviews__content">
          <div class="reviews__body isActive">
            <ReviewComp :review="this.GET_REVIEWS[0]"/>
          </div>  
        </div>
      </div>
      <div class="reviews__box">
        <ReviewComp class="reviews__box_el" v-for="review in this.GET_REVIEWS" 
          :key="review.id"
          :review="review"
        />
      </div> -->
    </section>
    <div class="subscribe center">
      <div class="subscribe__content">
        <h2 class="subscribe__title">GET 50%</h2>
        <form class="subscribe__form">
          <input class="subscribe__input" id="input"
            type="email" placeholder="Enter Yaur Email Address"/>
          <button class="subscribe__btn">SUBSCRUBE</button>
        </form>
      </div>
    </div> 
  </div>
</template>

<script>

import RestauransCard from '@/components/RestaurantsCard.vue';
import DishesCard from '@/components/DishesCard.vue';
import LineComp from '@/components/LineComp.vue';
import BtnViewAll from '@/components/BtnViewAll.vue';
import OrdersCard from '@/components/OrdersCard.vue';
import ReviewComp from '@/components/ReviewComp.vue';
import { mapActions, mapMutations, mapGetters } from 'vuex';

export default {
  name: 'HomeView',
  components: {
    RestauransCard,
    LineComp,
    DishesCard,
    BtnViewAll,
    OrdersCard,
    ReviewComp,
  },
  data() {
    return {
    }
  },

  computed: {
    ...mapGetters(['GET_RESTAURANS', 'GET_DISHES', 'GET_ORDERS', 'GET_REVIEWS']),
  },

  methods: {
    ...mapMutations(['SET_RESTAURANS', 'SET_DISHES', 'SET_ORDERS', 'SET_REVIEWS']),
    ...mapActions(['fetchRestaurans', 'feachDishes', 'feachOrders', 'feachReviews']),
  },

  mounted() {
    this.fetchRestaurans();
    this.feachDishes();
    this.feachOrders();
    this.feachReviews();
  }
}
</script>

<style lang="scss" scoped>
// Home
.home {
  &__top {
    display: flex;
    gap: 248px;
    margin-top: 119px;

    &_pseudo-box{
      position: relative;
      &::after{
        content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='384' height='221' viewBox='0 0 384 221' fill='none'%3e%3cpath opacity='0.2' d='M369.68 577.878C375.051 546.413 382.885 516.817 380.878 482.89C379.444 458.658 373.443 434.809 361.905 412.054C340.393 369.626 312.272 329.18 278.84 296.952C244.679 264.021 211.142 228.189 172.566 202.9C136.7 179.387 98.446 159.962 63.4785 134.41C4.88972 91.5978 -36.5779 23.2802 -71.7742 -42.2882C-90.3944 -76.9764 -108.862 -111.901 -126.689 -147.001C-137.875 -169.027 -147.273 -192.647 -161.028 -213.115' stroke='%23A596FF' stroke-width='4.01667' stroke-linecap='round'/%3e%3c/svg%3e");
        stroke-width: 4.017px;
        stroke: #A596FF;
        opacity: 0.5;
        position: absolute;
        top: -220px;
        left: 0; 
        z-index: 10;   
      }
      &::before{
        content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='428' height='221' viewBox='0 0 428 221' fill='none'%3e%3cpath opacity='0.2' d='M413.34 566.6C419.134 532.606 427.589 500.633 425.415 463.977C423.861 437.797 417.374 412.03 404.907 387.442C381.662 341.6 351.28 297.896 315.163 263.07C278.258 227.484 242.027 188.764 200.355 161.433C161.61 136.023 120.288 115.028 82.5135 87.4146C19.2219 41.1481 -25.5823 -32.6707 -63.6122 -103.518C-83.7315 -140.999 -103.686 -178.735 -122.948 -216.66C-135.036 -240.46 -145.191 -265.98 -160.052 -288.097' stroke='%23A596FF' stroke-width='4.01667' stroke-linecap='round'/%3e%3c/svg%3e");
        stroke-width: 4.017px;
        stroke: #A596FF;
        opacity: 0.5;
        position: absolute;
        top: -220px;
        left: 0px;
        z-index: 10;
      }
    }
  }
}

// Top Left
.top-left {
  &__box {
    display: flex;
    align-items: center;
    gap: 20px;
    margin-bottom: 18px;
  }

  &__line {
    width: 54px;
    height: 1px;
    opacity: 0.2;
    background: #201F1F;
  }

  &__over {
    color: rgba(32, 31, 31, 0.20);
    font-family: Poppins;
    font-size: 15px;
    font-style: normal;
    font-weight: 600;
    line-height: 120%;
    /* 18px */
    letter-spacing: 2px;
  }

  &__title {
    color: #201F1F;
    font-family: Poppins;
    font-size: 75px;
    font-style: normal;
    font-weight: 600;
    line-height: 120%;
    /* 90px */
    letter-spacing: -3px;
  }

  &__title>span {
    color: #5C4EAE;
    font-family: Poppins;
    font-size: 75px;
    font-style: normal;
    font-weight: 600;
    line-height: 120%;
    letter-spacing: -3px;
  }

  &__text {
    width: 499px;
    color: #676767;
    text-align: justify;
    /* Inter/Regular/18 */
    font-family: Inter;
    font-size: 18px;
    font-style: normal;
    font-weight: 400;
    line-height: 28px;
    /* 155.556% */
    margin-bottom: 53px;
  }

  &__text>span {
    opacity: 0.7;
    opacity: 0.7;
    color: #6155AE;
    font-family: Inter;
    font-size: 18px;
    font-style: normal;
    font-weight: 500;
    line-height: 28px;
  }

  &__btns {
    display: flex;
    gap: 18px;
    margin-bottom: 33px;

    &_started {
      // width: 156px;
      // height: 59px;
      padding: 18px 31px 17px 30px;
      color: #FFF;
      font-family: Poppins;
      font-size: 16.271px;
      font-style: normal;
      font-weight: 500;
      line-height: normal;
      letter-spacing: 0.163px;
      text-transform: capitalize;
      flex-shrink: 0;
      border-radius: 12.412px;
      background: #6C5FBC;
    }

    &_goPro {
      padding: 18px 28px 17px 27px;
      color: #6C5FBC;
      font-family: Poppins;
      font-size: 16.271px;
      font-style: normal;
      font-weight: 500;
      line-height: normal;
      letter-spacing: 0.163px;
      text-transform: capitalize;
      // width: 110px;
      // height: 59px;
      flex-shrink: 0;
      border-radius: 12.412px;
      border: 1.5px solid #6C5FBC;
    }
  }

  &__stars {
    display: flex;
    align-items: center;

    &_svg {
      flex-shrink: 0;
      margin-right: 20px;
    }

    &_svg-balls {
      opacity: 0.8;
      margin-right: 9.2px;
      margin-top: 5px;
    }

    &_balls,
    &_balls>span {
      color: var(--blue-gray-700, #334155);
      /* PJ/Semi-bold/16px */
      font-family: "Plus Jakarta Sans";
      font-size: 16px;
      font-style: normal;
      font-weight: 500;
      line-height: 24px;
      /* 150% */
      opacity: 0.7;
      margin-top: 7px;
    }
  }

}

// Top Right
.top-right {
  position: relative;
  &__card{
    width: 328px;
    filter: drop-shadow(-9.844px 12.305px 20.508px rgba(0, 0, 0, 0.15));
    position: absolute;
    z-index: 100;
    top: -50px;
    right: -500px;
    &::after{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='42' height='28' viewBox='0 0 42 28' fill='none'%3e%3cpath d='M3.03562 12.8839C6.63998 9.20371 11.0587 8.59002 15.5406 8.17742C21.1348 7.66242 26.8271 7.46063 31.1569 1.99998' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M1.63661 23.2285C4.92103 18.5553 9.69807 15.7937 15.3404 15.1712C18.2926 14.8456 21.2864 15.1697 24.22 14.6123C30.8291 13.356 36.9687 9.34402 39.8474 3.13949' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M30.5218 19.2191C28.3334 20.6222 25.9587 21.6906 23.3509 21.9971C18.1575 22.6072 12.7177 21.7974 8.77443 25.9997' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
      position: absolute;
      top: -40px;
      right: -50px;
    }
  }

  &__img {
    position: absolute;
    display: flex;
    justify-content: center;
    align-items: center;
    width: 422px;
    height: 422px;
    flex-shrink: 0;
    border-radius: 424px;
    background: linear-gradient(137deg, #6C5FBC 4.83%, #5144A0 98.22%);
    &::after{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='101' height='98' viewBox='0 0 101 98' fill='none'%3e%3cpath d='M71.6674 10.7111C80.803 24.3855 86.0196 40.0142 80.7461 55.9952C75.9861 70.42 63.0132 78.9641 48.1158 68.2622C41.0635 63.1962 36.4872 52.9723 41.0691 45.3764C46.1738 36.9138 57.5029 44.4112 60.5762 50.1704C67.7546 63.6215 63.1038 81.7597 47.3119 89.9534C28.6107 99.6567 20.562 81.1081 20.7098 81.1037' stroke='%23665AB5' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M91.0256 14.5963C89.1011 14.9712 87.2152 14.5933 85.3117 14.3026C79.0611 13.3488 72.67 10.5339 68.7869 5.89197C71.0236 11.8454 71.6846 20.2492 66.9237 25.2906' stroke='%23665AB5' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
      position: absolute;
      top: 70px;
      right: -65px;
      z-index: -10;
    }
  }
  &__img>img{
    // background: cover no-repeat;    
    width: 426px;
    height: 426px;
    border-radius: 100%;
    flex-shrink: 0;
    margin: 32px 37px 0 0;
  }

  &__graph{
    position: absolute;
    bottom: -10px;
    left: -115px;
    z-index: 100;
    padding: 17px 0 15px;
    display: flex;
    flex-direction: column;
    justify-content: space-between;
    filter: drop-shadow(13.407px -16.759px 27.932px rgba(0, 0, 0, 0.15));
    width: 206.987px;
    height: 205.48px;
    flex-shrink: 0;
    border-radius: 16.759px;
    border: 0.278px solid var(--Greyscale-200, #EDF2F7);
    background: var(--Others-White, #FFF);
    box-shadow: 14.12px 18.827px 37.655px 0px rgba(26, 32, 44, 0.15);
    &::before{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='39' height='37' viewBox='0 0 39 37' fill='none'%3e%3cpath d='M27.3376 31.1198C30.2316 24.6503 32.3782 17.9383 34.9719 11.3638' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M7.92894 35.0005C15.8316 26.5431 22.9341 17.0933 29.461 7.55682' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M1.63457 15.8358C9.15324 13.2755 18.5471 6.43167 25.4724 1.99974' stroke='%23685BB8' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
      position: absolute;
      bottom: -45px;
      left: -45px;
    }

    &_main{
      display: flex;
      justify-content: center;
      align-items: center;
      gap: 17px;

    }
    &_el{
      color: var(--Gray-3, #828282);
      text-align: center;
      font-family: Poppins;
      font-size: 6.85px;
      font-style: normal;
      font-weight: 400;
      line-height: normal;
      letter-spacing: 0.068px;
    }
    &_el:first-child{
      padding: 7px 11px;
      color: var(--Text-Light, #FFF);
      border-radius: 14.655px;
      background: #6C5FBC;
    }
    &_day{
      display: flex;
      justify-content: center;
      align-items: center;
      gap: 22.5px;

    }
    &_item{
      color: #828282;
      font-family: Roboto;
      font-size: 6.351px;
      font-style: normal;
      font-weight: 400;
      line-height: normal;
    }
    &_item:first-child{
      color: #151515;
      padding: 5.75px 6.97px 4.55px 9.21px;
      border-radius: 16.759px;
      background: rgba(231, 231, 231, 0.92);
    }
    &_line{
      position: relative;
      &::after{
        content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='10' height='10' viewBox='0 0 10 10' fill='none'%3e%3cpath d='M8.58326 4.94316C8.58326 7.05191 6.99247 8.64248 5.17234 8.64248C3.35221 8.64248 1.76143 7.05191 1.76143 4.94316C1.76143 2.83442 3.35221 1.24385 5.17234 1.24385C6.99247 1.24385 8.58326 2.83442 8.58326 4.94316Z' fill='%23C5BFED' stroke='%236C5FBC' stroke-width='2.22012'/%3e%3c/svg%3e");
        position: absolute;
        top: 15px;
        right: 30px;
        z-index: 101;
      }
      &::before{
        content: "$409";
        padding: 1.83px 5.15px 2.08px 5.5px;
        color: #FFF;
        font-family: Poppins;
        font-size: 5.55px;
        font-style: normal;
        font-weight: 600;
        line-height: normal;
        border-radius: 16.759px;
        background: #6C5FBC;
        position: absolute;
        top: 5px;
        right: 10px;
        z-index: 101;
      }
    }
  }
}

// Media Top
@media(max-width: 1024px){
  .home-top{
    flex-direction: column;
    gap: 94px;
  }
}

// Features
.features{
  position: relative;
  margin-top: 130px;
  padding: 57px 0 54px 0;
  background: #5C4EAE;
  display: flex;
  justify-content: center;
  gap: 85px;

  &::after{
    content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='88' height='221' viewBox='0 0 88 221' fill='none'%3e%3cpath opacity='0.2' d='M258.001 -55.6357C233.819 -53.3776 207.556 -53.0235 183.993 -47.7427C160.937 -42.5754 136.655 -33.2008 116.37 -23.2998C78.9867 -5.05417 59.8405 20.4887 42.4704 50.9625C13.8045 101.253 3.42611 153.413 2.9781 208.059C2.68151 244.235 4.07186 280.042 7.84701 316.1C8.9996 327.109 8.42353 339.694 16.5028 349.2' stroke='%23A596FF' stroke-width='4.44874' stroke-linecap='round'/%3e%3c/svg%3e");
    stroke-width: 4.449px;
    stroke: #A596FF;
    opacity: 0.5;
    position: absolute;
    top: 0;
    right: 0px;
  }
  &::before{
    content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='128' height='221' viewBox='0 0 128 221' fill='none'%3e%3cpath opacity='0.2' d='M250.547 -80.8447C227.068 -78.6528 201.569 -78.3091 178.691 -73.183C156.305 -68.1671 132.729 -59.0673 113.033 -49.4564C76.737 -31.7454 58.1474 -6.95103 41.2821 22.6297C13.4496 71.4467 3.3729 122.078 2.93791 175.123C2.64995 210.239 3.99988 244.997 7.66528 279.998C8.78437 290.684 8.22504 302.9 16.0695 312.128' stroke='%23A596FF' stroke-width='4.44874' stroke-linecap='round'/%3e%3c/svg%3e");
    stroke-width: 4.449px;
    stroke: #A596FF;
    opacity: 0.5; 
    position: absolute;
    top: 0;
    right: 10px;
  }

  &__item:not(:last-child){
    border-right: 1px solid rgba(197, 197, 197, .15);
    padding-right: 85px;
  }

  &__heading{
    color: var(--base-white, #FFF);
    text-align: center;
    font-family: "Public Sans";
    font-size: 43.2px;
    font-style: normal;
    font-weight: 700;
    line-height: 54px; /* 125% */
    margin-bottom: 11px;
  }
  &__text{
    color: #C5BFED;
    text-align: center;
    font-family: Inter;
    font-size: 14.4px;
    font-style: normal;
    font-weight: 500;
    line-height: 21.6px; /* 150% */
  }
}

// Download
.dowload{
  padding-top: 120px;
  padding-bottom: 220px;
  display: flex;
  align-items: center;
  justify-content: space-between;
  // gap: 287px;
  &__img-box{
    margin-left: 80px;
    width: 304px;
    position: relative;
    &::after{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='56' height='62' viewBox='0 0 56 62' fill='none'%3e%3cpath d='M29.4019 8.28731C19.9957 12.6113 11.1774 17.9211 2.00004 22.6496' stroke='%236C5FBC' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M54.464 22.7004C37.6407 24.3888 20.7416 27.9376 4.39179 32.1357' stroke='%236C5FBC' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M42.9382 49.8177C32.2122 45.3954 15.3032 43.8755 3.42795 42.0809' stroke='%236C5FBC' stroke-width='2.65' stroke-miterlimit='1.5' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
      position: absolute;
      top: -20px;
      left: 315px;
    }
  }

  &__content{
    max-width: 560px;
    position: relative;
    display: flex;
    flex-direction: column;
    justify-content: center;
    gap: 66px;
    &::after{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='169' height='61' viewBox='0 0 169 61' fill='none'%3e%3cpath d='M168.51 6.14524C168.736 5.44942 168.356 4.70168 167.66 4.4751C166.964 4.24854 166.216 4.62893 165.99 5.32475L168.51 6.14524ZM134.211 38.1393L134.417 39.4481L134.211 38.1393ZM94.9327 16.2392L96.2576 16.2287L94.9327 16.2392ZM106.699 2.00495L106.79 3.32679L106.699 2.00495ZM119.187 12.3894L117.941 12.8395L119.187 12.3894ZM85.8664 47.5723L86.0118 48.8893L85.8664 47.5723ZM50.0898 25.0035L51.4127 24.9278L50.0898 25.0035ZM58.6008 14.038L58.9272 15.3222L58.6008 14.038ZM32.8261 59.2674L32.9313 57.9466L32.8261 59.2674ZM1.37233 39.7189L1.67289 38.4284C1.20208 38.3187 0.708912 38.4742 0.386083 38.834C0.0632658 39.1938 -0.0379848 39.7009 0.121904 40.1571L1.37233 39.7189ZM12.9722 43.781C13.6849 43.947 14.3973 43.5038 14.5633 42.7911C14.7293 42.0784 14.2861 41.3661 13.5734 41.2001L12.9722 43.781ZM4.36582 52.266C4.60787 52.9566 5.3639 53.3202 6.05449 53.0782C6.74509 52.8362 7.10871 52.0801 6.86667 51.3895L4.36582 52.266ZM167.25 5.735C165.99 5.32475 165.99 5.32442 165.99 5.32421C165.99 5.32428 165.99 5.32417 165.99 5.32431C165.99 5.3246 165.99 5.32531 165.989 5.32646C165.989 5.32876 165.987 5.33277 165.985 5.33847C165.982 5.34989 165.975 5.36806 165.967 5.39279C165.95 5.44226 165.924 5.51796 165.889 5.61827C165.819 5.8189 165.711 6.11784 165.564 6.50209C165.271 7.27077 164.824 8.37969 164.214 9.72495C162.993 12.4182 161.123 16.0443 158.531 19.7794C153.33 27.276 145.345 35.0415 134.004 36.8304L134.417 39.4481C146.782 37.4974 155.317 29.0607 160.708 21.2901C163.413 17.3916 165.358 13.6179 166.628 10.8196C167.263 9.41912 167.73 8.2594 168.041 7.44588C168.196 7.03902 168.311 6.71848 168.389 6.49739C168.428 6.38683 168.457 6.30111 168.477 6.24188C168.487 6.21227 168.495 6.18927 168.5 6.1731C168.503 6.16501 168.505 6.15862 168.507 6.15398C168.508 6.15165 168.508 6.14976 168.509 6.1483C168.509 6.14757 168.509 6.14681 168.509 6.14644C168.509 6.14579 168.51 6.14524 167.25 5.735ZM134.004 36.8304C125.148 38.2275 115.647 36.5955 108.382 32.7941C101.099 28.9829 96.3124 23.1503 96.2576 16.2287L93.6077 16.2497C93.6732 24.5238 99.3941 31.0818 107.154 35.1421C114.932 39.212 125.01 40.9322 134.417 39.4481L134.004 36.8304ZM96.2576 16.2287C96.2416 14.2002 97.0816 11.0373 98.8904 8.32693C100.682 5.64226 103.304 3.56809 106.79 3.32679L106.607 0.683114C102.057 0.998027 98.7776 3.72199 96.6862 6.85591C94.6119 9.96411 93.587 13.6403 93.6077 16.2497L96.2576 16.2287ZM106.79 3.32679C110.087 3.09861 112.447 4.40049 114.211 6.284C116.015 8.20966 117.182 10.7376 117.941 12.8395L120.433 11.9393C119.629 9.71127 118.313 6.78656 116.145 4.47227C113.938 2.11582 110.836 0.390421 106.607 0.683114L106.79 3.32679ZM117.941 12.8395C120.967 21.2178 117.74 28.9549 111.233 34.9948C104.704 41.0555 95.0158 45.2291 85.721 46.2553L86.0118 48.8893C95.8117 47.8074 106.047 43.4237 113.036 36.937C120.047 30.4296 123.933 21.6293 120.433 11.9393L117.941 12.8395ZM85.721 46.2553C77.749 47.1355 69.3253 45.6514 62.824 41.9973C56.3496 38.3582 51.8532 32.6242 51.4127 24.9278L48.767 25.0792C49.2704 33.875 54.4526 40.3319 61.5256 44.3074C68.5717 48.2677 77.5643 49.822 86.0118 48.8893L85.721 46.2553ZM51.4127 24.9278C51.2745 22.5139 51.8549 20.4733 53.06 18.8788C54.2673 17.2812 56.1875 16.0186 58.9272 15.3222L58.2744 12.7539C55.074 13.5674 52.5855 15.1114 50.9458 17.281C49.3038 19.4536 48.5988 22.1407 48.767 25.0792L51.4127 24.9278ZM58.9272 15.3222C61.4325 14.6854 63.4587 15.1941 65.1694 16.4251C66.9338 17.6947 68.426 19.7842 69.6426 22.384L72.0428 21.2608C70.7277 18.4505 68.9935 15.912 66.7172 14.2741C64.3872 12.5975 61.5583 11.9191 58.2744 12.7539L58.9272 15.3222ZM69.6426 22.384C73.7169 31.0908 69.408 40.2171 61.3002 47.2331C53.2116 54.2323 41.8197 58.6546 32.9313 57.9466L32.7209 60.5882C42.4857 61.3661 54.5525 56.5765 63.0342 49.237C71.4965 41.9143 76.8694 31.5752 72.0428 21.2608L69.6426 22.384ZM32.9313 57.9466C26.0317 57.397 18.4366 52.7361 12.4277 48.014C9.45504 45.6779 6.93009 43.3751 5.14816 41.6558C4.25787 40.7968 3.55474 40.0851 3.07579 39.5899C2.83636 39.3423 2.65307 39.1489 2.53054 39.0185C2.46928 38.9533 2.42322 38.9038 2.39295 38.8711C2.3778 38.8548 2.36661 38.8426 2.35943 38.8348C2.35585 38.8309 2.35327 38.8281 2.3517 38.8264C2.35091 38.8256 2.35038 38.825 2.35011 38.8247C2.34997 38.8245 2.34996 38.8245 2.34988 38.8244C2.34994 38.8245 2.35004 38.8246 1.37233 39.7189C0.394615 40.6131 0.39485 40.6134 0.395158 40.6137C0.395344 40.6139 0.395711 40.6143 0.396083 40.6147C0.396828 40.6155 0.397834 40.6166 0.39909 40.618C0.401613 40.6208 0.405156 40.6246 0.409706 40.6295C0.418806 40.6394 0.431948 40.6537 0.449073 40.6722C0.48331 40.7091 0.533445 40.763 0.598903 40.8327C0.729812 40.9721 0.922037 41.1748 1.17093 41.4321C1.66864 41.9468 2.39344 42.6803 3.30813 43.5628C5.1362 45.3267 7.72958 47.6923 10.7903 50.0976C16.8481 54.8582 24.9799 59.9716 32.7209 60.5882L32.9313 57.9466ZM1.07177 41.0093L12.9722 43.781L13.5734 41.2001L1.67289 38.4284L1.07177 41.0093ZM0.121904 40.1571L4.36582 52.266L6.86667 51.3895L2.62275 39.2806L0.121904 40.1571Z' fill='%236C5FBC'/%3e%3c/svg%3e");
      position: absolute;
      bottom: -50px;
      right: 230px;
    }
  }

  &__title{
    color: var(--gray-900, #18181B);
    font-family: Poppins;
    font-size: 50.923px;
    font-style: normal;
    font-weight: 700;
    line-height: 58.198px; /* 114.286% */
    text-transform: capitalize;
  }
  &__title>span{
    color: #6C5FBC;
  }

  &__ul{
    margin-left: 1.3em;
  }

  &__li{
    list-style: inside;
    color: #676767;
    font-family: Inter;
    font-size: 20.335px;
    font-style: normal;
    font-weight: 400;
    line-height: 30.502px; /* 150% */
    text-indent: -1.3em;
    max-width: 558px;
  }
  &__li:not(:last-child){
    margin-bottom: 30px;
  }

  &__btn{
    max-width: 186px;
    padding: 20px 68px 20px 26px;
    color: #FFF;
    text-align: center;
    font-family: Poppins;
    font-size: 18.062px;
    font-style: normal;
    font-weight: 500;
    line-height: 22.577px; /* 125% */
    text-transform: capitalize;
    border-radius: 14.675px;
    background: #6C5FBC;
    position: relative;
    &::after{
      content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='28' height='28' viewBox='0 0 28 28' fill='none'%3e%3cpath d='M6.55469 13.5469H22.3588' stroke='white' stroke-width='2.25773' stroke-linecap='round' stroke-linejoin='round'/%3e%3cpath d='M14.4561 5.64453L22.3581 13.5466L14.4561 21.4487' stroke='white' stroke-width='2.25773' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
      position: absolute;
      top: 16px;
      right: 22px;
    }
  }
}
 
// Restaurans
.restaurans {
  padding-top: 100px;
  padding-bottom: 100px;

  &__title {
    color: #323142;
    font-feature-settings: 'clig' off, 'liga' off;
    font-family: Poppins;
    font-size: 45px;
    font-style: normal;
    font-weight: 600;
    line-height: 25.558px;
    /* 56.797% */
    text-align: center;
    margin-bottom: 85px;
  }

  &__title>span {
    color: #6C5FBC;
    font-feature-settings: 'clig' off, 'liga' off;
    font-family: Poppins;
    font-size: 45px;
    font-style: normal;
    font-weight: 600;
    line-height: 25.558px;
  }

  // &__btn {
  //   color: var(--Dark-Grey, #ACADB9);
  //   font-feature-settings: 'clig' off, 'liga' off;
  //   font-family: Poppins;
  //   font-size: 20px;
  //   font-style: normal;
  //   font-weight: 500;
  //   line-height: 45.184px;
  //   /* 225.921% */
  //   letter-spacing: -0.1px;
  //   position: relative;
  //   margin-left: 1135px;
  //   &::after {
  //     content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='19' height='4' viewBox='0 0 19 4' fill='none'%3e%3cpath d='M2 1.90234H17.8041' stroke='%23ACADB9' stroke-width='2.25773' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
  //     position: absolute;
  //     top: -5px;
  //     left: 88px;
  //   }

  //   &::before {
  //     content: url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns='http://www.w3.org/2000/svg' width='11' height='19' viewBox='0 0 11 19' fill='none'%3e%3cpath d='M1.90234 2L9.80441 9.90206L1.90234 17.8041' stroke='%23ACADB9' stroke-width='2.25773' stroke-linecap='round' stroke-linejoin='round'/%3e%3c/svg%3e");
  //     position: absolute;
  //     top: 2px;
  //     left: 99px;
  //   }
  // }
}

.cardList {
  display: flex;
  margin-bottom: 65px;
  gap: 30.37px;
}

// Dishes
.dishes{
  padding-top: 100px;
  padding-bottom: 120px;
  display: flex;
  flex-direction: column;
  // align-items: center;

  // &__btn{
  //   display: flex;
  //   justify-content: flex-end;
  //   padding-right: 40px;
  // }

  &__title{
    color: #323142;
    font-feature-settings: 'clig' off, 'liga' off;
    font-family: Poppins;
    font-size: 45px;
    font-style: normal;
    font-weight: 600;
    line-height: 25.558px; /* 56.797% */
    margin-bottom: 76px;
  }

  &__title>span{
    color: #6C5FBC;
  }

  &__cardList{
    display: flex;
    gap: 30.5px;
    margin-bottom: 66px;
    flex-wrap: wrap;
  }
}

// Dashboard
.dashboard{
  padding-top: 103px;
  padding-bottom: 138px;
  display: flex;
  justify-content: space-between;
  align-items: center;

  &__title{
    color: var(--gray-900, #18181B);
    font-family: Poppins;
    font-size: 50.923px;
    font-style: normal;
    font-weight: 700;
    line-height: 58.198px; /* 114.286% */
    text-transform: capitalize;
    margin-bottom: 52px;
  }

  &__title>span{
    color: #6C5FBC;
  }

  &__ordersCardList{
    display: flex;
    flex-direction: column;
    gap: 15px;
  }

  &__purchases{
    padding: 46px 38px 46px 38px;
    border-radius: 25.129px;
    border: 1.571px solid var(--Greyscale-200, #EDF2F7);
    background: var(--Others-White, #FFF);
  }
}

.purchases{
  &__top{
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 45px;
  }

  &__title{
    color: var(--Greyscale-900, #1A202C);
    font-family: Manrope;
    font-size: 28.271px;
    font-style: normal;
    font-weight: 800;
    line-height: 135%; /* 38.165px */
    letter-spacing: 0.314px;
    margin-left: 4px;
  }

  &__summary{
    display: flex;
    align-items: center;
    gap: 6px;
    padding: 0 18px 13px 0;
    cursor: pointer;
  }

  &__details{
    box-sizing: border-box;
    z-index: 10;
    padding-top: 10px;

    &[open] .purchases__details{

    }
  }

  &__select-title{
    color: var(--Greyscale-900, #1A202C);
    text-align: right;
    font-family: Manrope;
    font-size: 18.847px;
    font-style: normal;
    font-weight: 600;
    line-height: 150%; /* 28.271px */
    letter-spacing: 0.628px;
  }

  &__block{
    padding: 25px;
    width: 460px;
    display: flex;
    flex-direction: column;
    margin-bottom: 20px;
    border-radius: 18.847px;
    border: 1.571px solid var(--Greyscale-200, #EDF2F7);
  }
  &__block.expense{
    margin-bottom: 22px
  }

  &__content{
    display: flex;
    justify-content: space-between;
    margin-bottom: 20px;
  }

  &__content-box{
    display: flex;
  }

  &__box{
    display: flex;
    flex-direction: column;
    align-items: flex-start;
  }

  &__icon{
    border-radius: 1570.588px;
    background: #F4F0FF;
    margin-right: 19px;
  }
  &__icon:first-child{
    padding: 11.78px 12.56px 13.35px 12.56px;
  }
  &__icon:last-child{
    padding: 13.35px 13.56px 11.78px 12.56px;
  }

  &__title{
    color: var(--Greyscale-900, #1A202C);
    font-family: Manrope;
    font-size: 18.847px;
    font-style: normal;
    font-weight: 800;
    line-height: 150%; /* 28.271px */
    letter-spacing: 0.628px;
    margin-bottom: 3.5px;
  }

  &__text{
    color: var(--Greyscale-500, #A0AEC0);
    font-family: Manrope;
    font-size: 15.706px;
    font-style: normal;
    font-weight: 500;
    line-height: 150%; /* 23.559px */
    letter-spacing: 0.628px;
    margin-left: 4px;
  }

  &__line{
    width: 408px;
    height: 9.5px;
    border-radius: 78.529px;
  }
  &__expenseLine{
    width: 296px;
    height: 9.5px;
    border-radius: 78.529px;
    background: #6C5FBC;
  }

  &__line.expense{
    background: rgba(108, 95, 188, .2);
  }

  &__line.vocher{
    background: rgba(251, 173, 24, .2);
  }

  &__vocherLine{
    width: 296px;
    height: 9.5px;
    border-radius: 78.529px;
    background: #FBAD18;
  }

  &__price{
    color: var(--Greyscale-900, #1A202C);
    text-align: right;
    font-family: Manrope;
    font-size: 21.988px;
    font-style: normal;
    font-weight: 800;
    line-height: 150%; /* 32.982px */
    letter-spacing: 0.314px;
  }
}

// Reviews
.reviews{
  padding-top: 116px;
  padding-bottom: 217px; 
  // overflow: hidden;

  &__title{
    text-align: center;
    color: #323142;
    font-feature-settings: 'clig' off, 'liga' off;
    font-family: Poppins;
    font-size: 45px;
    font-style: normal;
    font-weight: 600;
    line-height: 25.558px;
    margin-bottom: 80px;
  }
  &__title>span{
    color: #6C5FBC;
  }

  &__wrapper{
    position: relative;
    display: flex;
    gap: 46px;
    flex-wrap: nowrap;
    align-items: flex-start;
    // flex: 0 0 100%;
    // display: flex;
    overflow: hidden;
  }

  &__container{
    flex: 0 0 100%;
    // display: flex;
  }

  &__content{
    max-width: 1248px;
    // margin: 0 auto;
    display: flex;
    align-items: center;
  }

  &__body{
    width: 46%;
  }

  &__box{
    // position: absolute;
    // top: 0;
    // left: 0%;
    // left: 46%;
    // width: 56%;
    // max-width: 100%;
    display: flex;
    flex-direction: column;
    // flex: 0 0 100%;
    // gap: 46px;
    // overflow-y: hidden;
    // overflow-x: scroll;
    // flex-wrap: nowrap;
    transform: translete(-100%, 0px);
  }
  &__box_el{
    position: absolute;
    display: flex;
    flex: 0 0 100%;
    top: 0px;
    left: 46%;
    flex-wrap: nowrap;
    gap: 46px;
    // transform: translete(-100%, 0px);
    // display: flex;
    // gap: 46px;
    // overflow-y: hidden;
    // overflow-x: scroll;
    // flex-wrap: nowrap;
  }
}

@media(max-width: 1200px){
  .reviews{
    &__wrapper{
      // gap: 4%;
    }
    &__box{
      // left: 50%;
    }

    &__body{
      // width: 50%;
    }
  }
}

@media(min-width: 767px){
  .review{
    &__wrapper{
      // display: flex;
      // flex-wrap: nowrap
    }

    &__body{
      // width: 46%;
    }

    &__box{
      // position: relative;
      // flex: 0 0 56%;
      // transform: translete(-100%, 0px);
    }

    &__box_el{
      // width: 100%;
      // height: 100%;
    }
  }
}

// Subscribe
.subscribe{
  margin-bottom: 216px;

  &__content{
    position: relative;
    border-radius: 30px;
    background: #5C4EAE;
    background-image: url('@/assets/images/GroupMask.png');
    padding: 37px 0 61px 66px;
    &::after{
      content: '';
      z-index: 10;
      position: absolute;
      top: 64px;
      right: 72px;
      width: 274.057px;
      height: 276.015px;
      flex-shrink: 0;
      background-image: url('~@/assets/images/FoodSubscribe.png');
      background-size: 100% 100%;
    }
  }

  &__title{
    color: #FFF;
    font-family: Poppins;
    font-size: 70px;
    font-style: normal;
    font-weight: 800;
    line-height: normal;
    margin-bottom: 9px;
  }

  &__input{
    width: 440px;
    height: 68px;
    border-radius: 14.234px;
    background: #FFF;
    box-shadow: 0px 7.253px 45.333px 0px rgba(0, 0, 0, 0.10);
    padding-left: 22px;
  }

  &__form{
    width: 440px;
    position: relative;
  }
  &__btn{
    content: "subscrube";
    z-index: 10;
    position: absolute;
    top: 8px;
    right: 6px;
    color: #F7F8FA;
    font-family: Poppins;
    font-size: 14.752px;
    font-style: normal;
    font-weight: 500;
    line-height: normal;
    letter-spacing: 0.443px;
    text-transform: uppercase;
    padding: 15.41px 23.99px 16.03px 25.39px;
    border-radius: 11.253px;
    background: #6C5FBC;
  }
  &__input::placeholder{
    color: #878787;
    font-family: Poppins;
    font-size: 16.268px;
    font-style: normal;
    font-weight: 400;
    line-height: normal;
    letter-spacing: 0.488px;
    text-transform: capitalize;
    
  }
}

</style>
